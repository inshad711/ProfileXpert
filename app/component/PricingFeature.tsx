...
export default function PricingFeature() {
    ...
    return (
        <section id="comparison-table" className="py-20 px-4 bg-gray-100">
            <div id="pricing" className="container mx-auto max-w-6xl">
                <div className={`text-center mb-12 ${isVisible ? "animate-slide-up" : "opacity-0"}`}>
                    <h2 className="text-3xl font-gotu lg:text-4xl mb-4">Free vs Premium Plan Comparison</h2>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1">
                        ...
                        >
                            ...
                            <div className="p-6 font-gotu space-y-4">
                                ...
                                <div className="text-center space-y-2 p-4 rounded-lg transition-all duration-300 hover:bg-purple-50 border-2 border-transparent hover:border-purple-200">
                                    <h3 className="text-2xl font-bold">Premium</h3>
                                    <p className="text-3xl font-bold text-purple-600">
                                        ₹49<span className="text-sm font-normal">/month</span>
                                    </p>
                                    <p className="text-sm text-gray-500">OR ₹399/year</p>
                                    {/* <button className="w-full bg-transparent border border-[#9929EA] text-purple-600 py-2 rounded-md hover:bg-gradient-to-r from-[#9112BC] to-[#9929EA] hover:text-white transition-all duration-300 hover:scale-105 hover:shadow-lg">
                                        Get Premium Plan
                                    </button> */}
                                    <a
                    ...
                    <div className="lg:col-span-2 overflow-x-hidden">
                        ...
                        >
                            <div className="overflow-x-hidden ">
                                <table className="w-full max-w-full table-auto text-xs sm:text-sm">
                                    <thead>
                                        <tr className="border-b font-gotu bg-gray-50">
                                            <th className="text-left p-3 sm:p-4 font-semibold">Feature</th>
                                            <th className="text-center p-3 sm:p-4 font-semibold">Free</th>
                                            <th className="text-center p-3 sm:p-4 font-semibold">Premium</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {features.map((feature, index) => (
                                            <tr
                                                key={index}
                                                className="border-b transition-all font-gotu cursor-pointer duration-300 hover:bg-purple-100 hover:scale-[1.005] transform"
                                                style={{ animationDelay: `${index * 0.05}s` }}
                                            >
                                                <td className="p-3 sm:p-4 font-medium break-words whitespace-normal">
                                                    {feature.name}
                                                </td>
                                                <td className="p-3 sm:p-4 text-center">
                                                    {typeof feature.free === "boolean" ? (
                                                        feature.free ? (
                                                            <Check className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 mx-auto transform transition-all duration-300 hover:scale-125" />
                                                        ) : (
                                                            <X className="w-4 h-4 sm:w-5 sm:h-5 text-red-500 mx-auto transform transition-all duration-300 hover:scale-125" />
                                                        )
                                                    ) : (
                                                        <span className="text-xs sm:text-sm">{feature.free}</span>
                                                    )}
                                                </td>
                                                <td className="p-3 sm:p-4 text-center">
                                                    {typeof feature.premium === "boolean" ? (
                                                        ...
                                                        <span className="text-xs sm:text-sm">{feature.premium}</span>
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
